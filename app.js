...
function convertGMTtoWIB(gmtTime) {
  ...
  let [_, hour, minute, period] = gmtTime.match(/(\d{1,2}):(\d{2})(am|pm)/i);
  hour = parseInt(hour);
    ...
    if (step1) {
      if (semuaBerita.length) {
        const daftar = semuaBerita.map(str => {
          ...
          const flag = getFlagEmoji(judul.includes(currency1) ? currency1 : currency2);
          return `• ${judul} (${jamWIB})\n  ${flag} ${efek}`;
        }).join("\n\n");
...
async function buatAnalisaSekarang() {
  ...
  setTimeout(() => typeText("typeWriter", result), 600);
}
